...
- 不进行重建后的视觉修复或视觉对比。
- 不要求用户逐页审批最终候选图。
- 不使用业务级哈希要求用户确认；内部仍使用本地 SHA-256 验证 UI revision、提示词、输入图片和候选文件，哈希不进入提示词且不产生模型额度。
...
- 每页 1–6 张参考图片为正常范围；7–10 张时 UI 强警告；11–16 张原则上要求用户删减；超过底层客户端允许的 16 张时禁止提交。任何层级都不得静默截断。
...
flowchart LR
    ...
    G --> H
    H --> I["用户一次性提交"]
    I --> J["冻结 result.json revision 与内部输入指纹"]
    J --> K{"本页存在可读确认图片"}
...
每张图片至少包含原始文件、实际模型输入文件和用途。原始文件用于 UI 查看与追踪；实际模型输入文件经过安全解码、方向修正、隐私元数据清理和尺寸归一化，用于控制输入成本与稳定性：

```json
{
  "original_path": "02_v6/reference_images/page_003/original/ref_001.png",
  "model_input_path": "02_v6/reference_images/page_003/model_input/ref_001.png",
  "purpose": "王巍讲话真实图片",
  "source": "web",
...
- 照片修正 EXIF 方向、删除 EXIF 和定位信息、保持比例并限制异常超大尺寸，不裁切主要人物。
- Logo 保留原始文件；SVG 必须经过安全光栅化生成模型输入 PNG，保持比例，不拉伸、不裁切。
- 截图使用无损 PNG，保证文字可读，不使用有损压缩；异常超大截图在保留可读性的前提下缩放。
- 解码前验证真实 MIME、文件大小和响应类型；解码时限制总像素，防止压缩炸弹或图片炸弹。
- 任何预处理失败都记录状态并回到 UI，不把损坏或不可信文件交给 Image2。
...
- Word 图片和附件图片的安全缩略图，点击后可查看项目目录中的完整原图。
- 搜索得到的真实图片缩略图、来源和完整原图查看入口。
- 每张图片同时显示“原图”和“实际送入 Image2 的归一化版本”，避免用户误解实际输入。
- 每张图片的用途、是否允许裁切和是否允许风格融合。
- 由批注转换的具体生图要求，可编辑。
- 无法满足或禁止生成的要求以及降级表达，可编辑。
- 当前提示词材料字符数和超限警告。
- 当前确认图片数量；超过 6 张显示成本与遵循度警告，超过 10 张显示强警告，超过 16 张禁止提交。

UI 可以显示原图，因为图片文件位于本地新项目目录。页面列表使用本地安全缩略图，点击后通过受项目目录限制的媒体接口查看完整原图；同时展示实际模型输入版本。媒体接口必须：

- 校验项目身份、会话 nonce 和文件路径归属。
- 对规范化后的绝对路径执行项目根目录包含检查，拒绝 `..`、符号链接逃逸和项目外路径。
- 验证真实 MIME、文件签名、文件大小和解码像素上限，不信任扩展名。
- 禁止直接以内联可执行形式提供 HTML 或不可信 SVG；SVG 只提供安全光栅化预览。
- 搜索下载设置连接与读取超时、重定向上限、响应大小上限，并拒绝本地、环回、私有网段和非 HTTP(S) 目标，防止 SSRF。
- 不把访问令牌、用户文件内容、原图或生成图片写入普通日志和发布包。
...
使用临时文件写入后原子替换，避免半写入。重新打开 UI 并再次提交时递增 `revision`。用户不需要查看或确认哈希，也不调用模型完成确认；后台在本地计算并保存：

- UI revision 内容 SHA-256。
- 每张实际模型输入图片 SHA-256。
- 最终提示词 SHA-256。
- 每个候选输出 SHA-256。

这些内部指纹不进入 Image2 请求，不消耗 OpenAI token，仅用于防止同一路径内容变化后错误复用旧 receipt。
...
- 普通文字、概念模型、照片为主且文字较少的页面，首次使用 `quality=medium`。
- Logo、截图、密集数据、小字较多或高细节保真页面，首次使用 `quality=high`。
- Medium 首版只有在语义视觉 QA 给出明确可修复问题时，才允许第二版升级为 High。
- 每页正常最多两个有效候选；不默认生成第三版。

质量判定在 UI 提交后由确定性页面风险规则完成，只能读取用户确认的材料类型、图片类型、文本长度、图表数量和图片用途，不得重新解释或改写用户内容。
...
- 不把上一版候选图作为输入。
- 第一版通过 QA 后立即停止，不生成第二版。
- 只有 QA 给出明确、可执行且未重复的反馈时才生成第二版。
- 第二版失败、无明显改善、重复相同问题或 QA 不可用时，按既有合同使用第一版有效候选。
- 默认最大候选数为 2。

### 8.5 并发与限流

- 默认最大并发为 2；速度模式允许 3，不再默认 5 或更高。
- 每个页面保持独立状态和 receipt，一个页面失败不得破坏其他页面。
- 收到 429 时降低并发到 1，并使用带抖动的指数退避；不得立即批量重试。
- 只重试 429、5xx、网络中断等瞬时错误；用户可修正错误、输入审核阻断和无效图片不得无修改自动重试。
- 已完成且 receipt 验证通过的页面在恢复运行时不得再次调用 Image2。

## 9. 最终提示词

提示词采用单一权威、去重的精简结构，仅由以下内容组成：
...
原始批注、Word 原文的重复副本、未确认附件内容、UI 展示元数据、搜索过程记录和后台推断不得进入提示词。UI 可以同时展示 Word 原文和有效正文，但 Image2 只接收用户确认的完整有效正文。每条约束只出现一次。
...
本层不调用语义模型，也不产生 Image2 额度：

- 输入文件存在且可安全解码。
- Generate 没有输入图片，Edit 至少有一张输入图片。
- 输入图片数量、格式、文件大小和像素满足合同。
- 输出存在、可解码且为 1904×896、17:8。
- 整体视觉合同非空。
- 提示词未超过 32,000 字符。
- UI revision、提示词、输入图片和候选输出的内部 SHA-256 与 receipt 一致。
- 固定页面主标题没有进入正文事实权限字段。

机械 QA 失败时不得调用语义视觉 QA，也不得用相同错误输入重新消耗 Image2；应先修正本地合同或记录明确技术失败。

### 10.2 第二层：一次轻量语义视觉 QA
...
只有语义视觉 QA 给出明确、可执行且与上一次不同的改进反馈时，才允许第二个候选。QA 不检查：
...
| 参考图片过多导致成本和遵循度下降 | 6 张软上限、10 张强警告、16 张硬上限；由用户在唯一确认阶段删减 |
| 超大照片或截图增加输入 token 和延迟 | 保存原图，同时生成安全、保持比例的模型输入版本 |
| 恶意 SVG、图片炸弹或伪造 MIME | 文件签名检查、像素与大小上限、安全光栅化、禁止不可信 SVG 内联 |
| 搜索下载造成 SSRF 或无限重定向 | 只允许 HTTP(S)，拒绝本地和私有网段，限制超时、重定向和响应大小 |
...
| 同路径图片内容变化后误用旧候选 | receipt 校验 operation、模型、UI revision、提示词 SHA-256、输入图片 SHA-256 和输出 SHA-256 |
| 高质量模式额度和延迟过高 | 普通页 Medium 首版，高风险页 High 首版，最多两个候选，QA 通过立即停止 |
| 高并发触发 429 | 默认并发 2，速度模式 3；429 降为 1 并指数退避 |
...
- UI 能点击查看完整原图，并同时显示实际送入 Image2 的归一化版本。
- 每页完整有效正文、附件提取结果、图表描述、生图要求和降级表达可编辑。
- 用户只需一次最终提交。
- 超过字符或图片数量上限时在提交前明确提示。
- 6 张以上显示成本与遵循度警告，10 张以上显示强警告，16 张以上禁止提交。
...
- 任何候选重试都不使用上一版候选图片。
- 普通页面首次使用 Medium，高风险页面首次使用 High；每页最多两个有效候选。
- QA 通过后不得继续生成候选。
...
- 同一路径图片内容变化、提示词变化或候选文件变化时，旧 receipt 必须通过内部 SHA-256 校验失效。
- 搜索失败不会重复搜索。
- 附件稳定解析结果不会重复解析。
- 默认并发为 2；速度模式最多 3；429 时降低并发并退避。
...
3. V6 Generate/Edit 自适应分流、自适应质量、限流退避、trace 和内部指纹恢复凭据。
4. 权威精简提示词、风格合同、分层 QA 和完整回归测试。
