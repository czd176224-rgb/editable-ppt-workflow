...
4. Edit or use reference images by passing one or more `--image` inputs. For edits, build the prompt from the user's requested changes and the invariants that must stay unchanged. When a governed workflow must prove which references were used, pair each input with `--image-role <role>` and add `--trace-out <trace.json>`; the trace records the endpoint, model, Codex OAuth mode, input hashes, and output hashes.

5. Report the saved path(s), model, size, and whether Codex OAuth was used. For an explicit `WIDTHxHEIGHT`, the CLI validates the returned aspect ratio and losslessly writes or high-quality resamples the final file to those exact pixel dimensions; it refuses mismatched aspect ratios instead of stretching the image.
