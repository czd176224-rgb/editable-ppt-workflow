...
def write_generation_trace(
    ...
    if (
        ...
        for path in outputs:
            ...
            if _ratio_delta(actual_size, requested_size) > 0.005:
                warnings.append({
                ...
                with Image.open(BytesIO(data)) as source:
                    ...
                    if actual_size != expected_size:
                        ...
                        if ratio_delta > 0.005:
                            if not allow_off_ratio_for_downstream_repair:
                                raise CliError(
                            ...
                            eprint(
                                f"Preserved off-ratio generated image {actual_size[0]}x{actual_size[1]} "
                                f"for downstream repair to requested {expected_size[0]}x{expected_size[1]}."
                            )
                        else:
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    written = write_images(
        extract_image_payloads(response),
        args.out,
        output_format,
        args.size,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_image_arguments(command: argparse.ArgumentParser, operation: str) -> None:
        ...
        command.add_argument("--trace-out", help="Write a JSON generation trace with endpoint, model, input hashes, and output hashes.")
