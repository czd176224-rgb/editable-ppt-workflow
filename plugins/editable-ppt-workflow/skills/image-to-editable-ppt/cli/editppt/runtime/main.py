...
""",
    ...
    record = run_sub.add_parser(
        "record",
        help="Seal and record one current editable Word page.",
        description="Create a current editable-page descriptor from a one-slide PPTX, seal it in the project cache, and complete the matching reconstruction lease.",
        formatter_class=HELP_FORMATTER,
    )
    record.add_argument("run", metavar="RUN", help="Current Word-only project directory.")
    record.add_argument("--page", required=True, type=int, metavar="PAGE", help="Locked Word page number.")
    record.add_argument("--agent-id", required=True, metavar="ID", help="Runtime worker/thread id that produced the result.")
    record.add_argument("--attempt", required=True, type=int, metavar="N", help="Active reconstruction lease attempt.")
    record.add_argument("--pptx", required=True, metavar="FILE", help="Project-local one-slide editable PPTX.")
    record.add_argument("--artifact", required=True, metavar="FILE", help="Project-local current page-package descriptor to write.")
