...
def package_identity_from_page(page, *, require_context=False):
    """Return the immutable Task-5-provided editable identity for a page."""
    try:
        from .editable_page_cache import PackageIdentity, PackageValidationError
    except ImportError:  # direct runtime script entrypoint
        from editable_page_cache import PackageIdentity, PackageValidationError

    payload = page.get("editable_package_identity")
    if payload is None:
        raise PackageValidationError(
            f"{page.get('page_id', 'page')} has no authoritative editable_package_identity"
        )
    identity = PackageIdentity.from_dict(payload)
    if require_context:
        context = page.get("context")
        keys = context.get("keys") if type(context) is dict else None
        if type(keys) is not dict or keys.get("editable") != identity.editable_key:
            raise PackageValidationError(
                f"{page.get('page_id', 'page')} context.keys.editable does not match PackageIdentity"
            )
    return identity


def required_workflow_contract(deck):
    value = deck.get("workflow_contract_version")
    if type(value) is not str or not value:
        raise ValueError("deck_manifest.json must explicitly declare workflow_contract_version")
    return value
