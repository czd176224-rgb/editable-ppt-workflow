...
def _load_current_run(project: Path) -> dict[str, Any]:
    ...
    if not isinstance(run, dict) or run.get("workflow_contract_version") != WORKFLOW_VERSION:
        raise PackageValidationError(f"page package requires workflow_contract_version {WORKFLOW_VERSION}")
    return run
