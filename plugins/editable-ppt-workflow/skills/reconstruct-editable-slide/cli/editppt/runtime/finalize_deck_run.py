...
try:
    ...
    from . import workflow_state_store
...
def _load_run(project: Path) -> dict[str, Any]:
    ...
    run = workflow_state_store.load_state(project)
    if not isinstance(run, dict) or run.get("workflow_contract_version") != CURRENT_CONTRACT:
        raise PackageValidationError(f"finalize accepts only workflow_contract_version {CURRENT_CONTRACT}")
    return run
...
def _replay_completion(
    ...
    current = workflow_state_store.load_state(project)
        ...
        try:
            with workflow_state_store.project_state_lock(project):
                ...
                try:
                    ...
                    workflow_state_store.replace_state(project, current)
                    _cleanup_staging(project, staging)
                    committed = True
                except BaseException as failure:
                    rollback_errors: list[BaseException] = []
                    try:
                        workflow_state_store.replace_state(project, original_state)
                    except BaseException as exc:
