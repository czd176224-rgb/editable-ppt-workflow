...
def cmd_record(args: argparse.Namespace) -> int:
    run_dir = run_dir_from_target(args.run)
    if (run_dir / "deck_manifest.json").is_file() and (run_dir / "page_jobs.json").is_file():
        return run_script(
    ...
    missing = [
        name for name, value in (
            ("--attempt", args.attempt), ("--pptx", args.pptx), ("--artifact", args.artifact)
        ) if value is None
    ]
    if missing:
        raise SystemExit("current Word project record requires " + ", ".join(missing))
    return run_script(
...
""",
    ...
    record = run_sub.add_parser(
        ...
        description="For a prepared manifest run, validate the required page artifacts and record the manifest as final assembly authority. Legacy Word projects retain their package-record path.",
        formatter_class=HELP_FORMATTER,
    )
    record.add_argument("run", metavar="RUN", help="Current Word-only project directory.")
    record.add_argument("--page", required=True, metavar="PAGE", help="Page id such as page_001, or a legacy Word page number.")
    record.add_argument("--agent-id", required=True, metavar="ID", help="Runtime worker/thread id that produced the result.")
    record.add_argument("--attempt", type=int, metavar="N", help="Legacy Word reconstruction lease attempt.")
    record.add_argument("--pptx", metavar="FILE", help="Legacy Word project-local one-slide editable PPTX.")
    record.add_argument("--artifact", metavar="FILE", help="Legacy Word page-package descriptor to write.")
