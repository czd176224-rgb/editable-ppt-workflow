...
def main():
    ...
    parser.add_argument(
        "--for-repair",
        action="store_true",
        help="Explicitly reopen an accepted page after final visual QA requests a targeted repair.",
    )
    ...
    if status == "accepted" and not args.for_repair:
        raise SystemExit(
            f"{page['page_id']} is accepted; use --for-repair only after final visual QA requests a targeted repair"
        )
    if status not in {"dispatched", "recorded", "accepted"}:
        raise SystemExit(
            f"{page['page_id']} cannot be reset from status {status}; "
            "reset only applies to dispatched, recorded, or explicitly repairable accepted pages"
        )
