...
from page_pipeline import cache_record  # noqa: E402
from editable_page_cache import seal_completed_page  # noqa: E402
from page_qa import PageQAResult  # noqa: E402
from native_page_builder import build_overlay_page  # noqa: E402
...
def _coverage_receipt(project: Path, page_number: int) -> Path:
...
def _project(
    ...
    *,
    canvas: str = "ppt169",
    identical_source_pages: bool = False,
) -> Path:
    if canvas != "ppt169":
        raise ValueError("fixed-canvas-cm-v2 only supports ppt169")
    project = tmp_path / "project"
    project.mkdir()
    jobs = []
    for page_number in locked_order:
        source = "两页共享的正文原文" if identical_source_pages else f"第{page_number}页唯一内容"
        page_title = f"Word page {page_number}"
        body_text = source
        contract = {
            "schema_version": "2.0",
            ...
            "source_blocks": [{
                "type": "paragraph", "text": source, "source_block_index": 1,
            ...
            "semantic_units": [{
                ...
                "source_trace": [{
                    "source_type": "word_page", "source_page": page_number,
            ...
            "page_purpose": "mechanical assembly fixture",
            "detected_dates": [],
            "detected_numbers": [],
            "detected_amounts": [],
    ...
    style = compile_style_execution({**confirmed_result(), "page_count": len(locked_order)})
    style_file = project / "02_style" / "style_execution.json"
    style_file.parent.mkdir()
    style_file.write_bytes(canonical_json_bytes(style))
    style_digest = hashlib.sha256(canonical_json_bytes(style)).hexdigest()
    (style_file.parent / "style_execution.sha256").write_text(style_digest + "\n", encoding="ascii")
    ...
    source_file = project / "00_source" / "source.docx"
    source_file.write_bytes(b"mechanical assembly fixture Word source")
    source_sha = hashlib.sha256(source_file.read_bytes()).hexdigest()
    run = {
        "schema_version": "1.0",
        "workflow_contract_version": "word-ppt-workflow-v4",
        "project_name": "mechanical-assembly",
        "word_source": {"path": "00_source/source.docx", "sha256": source_sha, "pages_path": "00_source/pages.json"},
        "logo_source": {"path": "00_source/company_logo.svg", "sha256": hashlib.sha256(logo.read_bytes()).hexdigest(), "media_type": "image/svg+xml"},
        "pagination": {
        ...
        "style_confirmation": {
            ...
            "execution_sha256": style_digest,
        },
        "jobs": jobs,
        "final_pptx": None,
        "scheduler": {"concurrency": 2, "configured_max": 2, "last_trigger": "test"},
        "runtime": {"generation_mode": "continuous", "image_quality": "high", "automatic_repair_budget": 1},
    }
    _write_json(project / "01_page_contracts" / "source_lock.json", {
        ...
        "pages": [{
            ...
            "contract_sha256": hashlib.sha256((project / job["contract_file"]).read_bytes()).hexdigest(),
...
def _page_pptx(
    ...
    *,
    ...
    presentation.slide_height = Inches((14.288 / 2.54) if canvas == "ppt169" else 7.5)
...
def test_v4_editable_page_recording_requires_a_current_project_state(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="workflow_run.json"):
        record_editable_page(
            tmp_path,
            1,
            "reconstructor-1",
            1,
            tmp_path / "page.pptx",
            editable_receipt=tmp_path / "editable-receipt.json",
        )


def test_v4_state_cli_does_not_advertise_legacy_reconstruction_recording() -> None:
    completed = subprocess.run(
        [sys.executable, str(SCRIPTS / "workflow_state.py"), "--help"],
        check=False,
        capture_output=True,
        text=True,
    )
    assert completed.returncode == 0
    assert "record-reconstruction" not in completed.stdout


def test_ppt43_project_is_rejected_by_the_single_canvas_contract(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="only supports ppt169"):
        _project(tmp_path, (1,), canvas="ppt43")
...
def test_final_assembly_has_exactly_one_slide_per_locked_word_page_in_locked_order(tmp_path: Path) -> None:
    project = _project(tmp_path, (1, 2, 3))
    _complete_all(project)

    receipt = assemble_deck(build_current_assembly_plan(project), project / "08_final" / "deck.pptx")

    assert receipt.slide_order == (1, 2, 3)
    assert receipt.page_count == 3
    assert _slide_texts(receipt.output_path) == ["Word page 1", "Word page 2", "Word page 3"]
    assert len(Presentation(receipt.output_path).slides) == 3
    assert editable_page_cache.inspect_editable_pptx(receipt.output_path).slide_page_ids == (1, 2, 3)
...
def test_finalizer_writes_only_current_mechanical_outputs(tmp_path: Path) -> None:
    ...
    metrics = json.loads((project / "09_reports" / "pipeline_metrics.json").read_text(encoding="utf-8"))
    assert metrics["state_revision"] == hashlib.sha256(
        (project / "workflow_run.json").read_bytes()
    ).hexdigest()
    assert metrics["final_pptx"] == "08_final/deck.pptx"
...
def test_identical_word_text_with_different_fixed_titles_keeps_distinct_page_packages(tmp_path: Path) -> None:
    project = _project(tmp_path, (1, 2), identical_source_pages=True)
    _complete_all(project)
    run = load(project)
    ...
    assert _slide_texts(receipt.output_path) == ["Word page 1", "Word page 2"]
...
def test_state_or_summary_stage_write_failure_publishes_nothing(
    ...
    project = _project(tmp_path, (1, 2))
    _complete_all(project)
    if failed_name == "workflow_run.json":
        real_replace = finalize_deck_run.workflow_state_store._replace_file
        calls = 0

        def injected_replace(project_root: Path, path: Path, value: dict) -> None:
            nonlocal calls
            calls += 1
            if calls == 1:
                raise OSError("injected workflow_run.json write failure")
            real_replace(project_root, path, value)

        monkeypatch.setattr(
            finalize_deck_run.workflow_state_store, "_replace_file", injected_replace
        )
    else:
...
def test_lock_release_failure_after_commit_does_not_rollback_outside_the_lock(
    ...
    project = _project(tmp_path, (1, 2))
    _complete_all(project)
    real_lock = finalize_deck_run.workflow_state_store.project_state_lock
    ...
    monkeypatch.setattr(
        finalize_deck_run.workflow_state_store, "project_state_lock", failing_exit_lock
    )
    summary = finalize_project(project, renderer=_render_with_real_open)
...
def test_project_and_cache_artifact_reparse_points_are_rejected_before_resolution(tmp_path: Path) -> None:
    project = _project(tmp_path, (1, 2))
    _complete_all(project)
    linked_project = tmp_path / "linked-project"
    with _directory_reparse_point(linked_project, project):
        with pytest.raises(PackageValidationError, match="link|reparse|project"):
            build_current_assembly_plan(linked_project)

    editable = project / "07_editable"
    replacement = tmp_path / "replacement-editable"
    editable.rename(replacement)
    try:
        with _directory_reparse_point(editable, replacement):
            with pytest.raises(PackageValidationError, match="link|reparse|artifact|path"):
                build_current_assembly_plan(project)
    finally:
        if not editable.exists():
            replacement.rename(editable)
...
def test_final_output_parent_reparse_point_cannot_redirect_publication(tmp_path: Path) -> None:
    project = _project(tmp_path, (1, 2))
    _complete_all(project)
    outside = tmp_path / "outside-final"
    outside.mkdir()
    with _directory_reparse_point(project / "08_final", outside):
