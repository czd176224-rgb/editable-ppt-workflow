(function () {
  ...
  var productionProfiles = [
    ...
    {id: "balanced", name: "均衡", note: "质量、速度和稳定性平衡", quality: "高", concurrency: 4, repair: 1, detail: "高质量生图 · 最多并发4页 · 每页最多自动修复1次"},
    {id: "speed", name: "速度优先", note: "适合页数较多、时间紧的材料", quality: "中", concurrency: 6, repair: 1, detail: "中等质量生图 · 最多并发6页 · 每页最多自动修复1次"}
  ];
  var fieldLabels = {
  ...
  var colorRoles = [
    ...
    {id: "primary", label: "页面标题", locked: true},
  ...
  function initializeState() {
    ...
    state.canvas = recommended.canvas || "ppt169";
  ...
  function renderStyleSummary() {
    ...
    var note = node("p", "summary-note", "精确色板和字体会锁定；其他选项为柔性设计偏好。 ");
  ...
  function renderDetailStep(panel) {
    ...
    form.append(imageRole, colorControls(), typographyControls());
  ...
  function renderProductionProfiles() {
    ...
    sectionNode.append(node("p", "eyebrow", "PRODUCTION & DELIVERY"), node("h3", "", "确认生产与交付机制"), node("p", "production-intro", "三档模式会直接映射为以下技术参数；所有模式均采用逐页独立生成、异常页单独重试、严格项目内缓存，并交付可编辑 PPT 和页面图片。"));
  ...
  function renderContractStep(panel) {
    ...
    panel.append(specification, renderProductionProfiles());
    ...
    boundary.append(node("h3", "", "合同怎样参与生成"), node("p", "", "精确颜色和字体作为视觉锚点；其他视觉选择作为柔性偏好。生产模式决定图像质量、并发页数和自动修复次数；确认后系统将连续执行，不再逐页询问。"));
  ...
  Promise.all([requestJson("/api/catalogs"), requestJson("/api/recommendations"), requestJson("/api/pages")]).then(function (values) {
