...
def resolve_command(command: str) -> str | None:
    ...
    home = Path.home()
    runtime = home / ".codex/plugin-runtimes/editable-ppt-workflow"
...
try:
    ...
    print(json.dumps({"slides": int(presentation.Slides.Count)}))
finally:
...
'''
    try:
        ...
        return {"available": False, "passed": False, "detail": "PowerPoint COM timed out after 45 seconds"}
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout).strip()[:500]
        return {
            "available": False,
    ...
    if slides != 1:
        return {
            "available": True,
            "passed": False,
            "detail": f"PowerPoint opened {slides} slides instead of 1",
        }
    return {"available": True, "passed": True, "detail": "PowerPoint opened 1 slide"}
...
def office_smoke_test(officecli_path: str | None, powerpoint_probe: dict | None = None) -> dict:
    if not officecli_path:
        return {"passed": False, "backend": None, "detail": "officecli not found"}
    try:
        with tempfile.TemporaryDirectory(prefix="editable-ppt-workflow-") as temporary_name:
            temporary = Path(temporary_name)
            pptx = temporary / "smoke.pptx"
            created = _create_officecli_smoke(officecli_path, pptx)
            if not created["passed"]:
                return {"passed": False, "backend": None, "detail": created["detail"]}
            ...
            if powerpoint["available"]:
                return {"passed": False, "backend": "powerpoint", "detail": powerpoint["detail"]}
        ...
        return {"passed": False, "backend": None, "detail": str(exc)}
...
'''
    try:
        ...
        return {"available": False, "detail": "PowerPoint status probe timed out after 15 seconds"}
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout).strip()[:500]
        return {
            "available": False,
...
def external_skill_status() -> dict:
    ...
    candidates.append(skill_root.parent / "codex-gpt-image")
    editppt_path = resolve_command("editppt")
    if editppt_path:
        candidates.append(Path(editppt_path).resolve().parent.parent / "codex-gpt-image")
...
def diagnose(check_powerpoint: bool = False, smoke_test: bool = False) -> dict:
    ...
    result["required_ok"] = bool(result["python"]["ok"] and all(modules.values()))
    ...
    result["workflow_ready"] = bool(
        ...
        and external["officecli_available"]
        and editppt_command["available"]
        and officecli_command["available"]
    ...
    if smoke_test:
        result["office_smoke_test"] = office_smoke_test(
            ...
            result["powerpoint"] if check_powerpoint else None,
        )
