...
def extract(input_path: Path, marker_pattern: str) -> dict:
    ...
    seen: set[int] = set()
    for source_block_index, block in enumerate(iter_blocks(Document(input_path))):
        if isinstance(block, Paragraph):
            ...
            if match:
                ...
                block_relationship_ids = relationship_ids(block)
                if text or block_relationship_ids:
                    current["blocks"].append(
                        {
                            "type": "paragraph",
                            "text": text,
            ...
            block_relationship_ids = relationship_ids(block)
            if markdown or block_relationship_ids:
                current["blocks"].append(
                    {
                        "type": "table",
                        "markdown": markdown,
