...
def test_powerpoint_status_probe_timeout_is_bounded_and_never_runs_com_in_parent(
    ...
    assert "timed out after 15 seconds" in result["detail"]
...
def test_powerpoint_open_timeout_falls_back_to_fake_soffice(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    assert result["passed"] is True
    assert result["backend"] == "libreoffice"
    assert "timed out after 45 seconds" in result["detail"]
...
def test_failed_bounded_status_probe_skips_a_second_com_attempt(
    ...
    result = doctor.office_smoke_test(
        str(officecli),
        {"available": False, "detail": "PowerPoint status probe timed out after 15 seconds"},
    )
...
def test_successful_office_smoke_is_authoritative_when_version_probe_times_out(
    ...
    monkeypatch.setattr(doctor, "powerpoint_status", lambda: {"available": False, "detail": "timed out"})
    monkeypatch.setattr(
        doctor,
        "office_smoke_test",
        lambda *_args, **_kwargs: {"passed": True, "backend": "libreoffice", "detail": "rendered"},
    )
    monkeypatch.setattr(
        doctor.subprocess,
        "run",
